...
public class EDLFile extends FeatFile //implements Cloneable
{
    public EDLFile(String fn)
    {
        ...
        // keep chunks somewhat sorted
        chunks = new MaxHeap(100);
    }
    ...
    public void readFile() throws IOException, ParserException
    {
        ...
        { 
            ...
            if(!p.find())
                continue;
            try { dstTime = Double.parseDouble(p.group(1)); }
            catch(NumberFormatException nfe) { 
                throw new ParserException(filename, lineno, 
                                          "Could not parse dest start time \"" 
                                          + p.group(1)  + "\".");  }
            ...
            try { srcTime = Double.parseDouble(p.group(1)); }
            catch(NumberFormatException nfe) { 
                throw new ParserException(filename, lineno, 
                                          "Could not parse source start time \"" 
                                          + p.group(1)  + "\".");  }
    ...
    /**
     * Get the matrix of features associated with each chunk in this
     * EDLFile
     */
    ...
    public double[][] getFeatures(int[] featdim)
    {
        ...
        // the above call destroys the Heap property in chunks, need
        // to fix it here.
        if(!((Heap)chunks).isHeap())
            ((MaxHeap)chunks).rebuildHeap();
	...
	public Object clone()
	{
		EDLFile o = new EDLFile(this.filename);
		...
		while(i.hasNext())
			o.chunks.add(((EDLChunk)i.next()).clone());
	...
	*/
    ...
    protected void write(Writer w) throws IOException
    {
        ...
        // the above call destroys the Heap property in chunks, need to fix it here.
        ((MaxHeap)chunks).rebuildHeap();
    }
}
