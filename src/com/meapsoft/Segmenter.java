...
//import featextractors.*;

/**
 * Program to segment input files and output text feature files
 * compatible with other MEAPsoft tools.
 *
...
public class Segmenter extends MEAPUtil
{
    ...
    public Segmenter(String[] inFiles, String outputFile, double thr, boolean beatOD, boolean oaff)
    {
   ...
   public Segmenter(String[] inFiles, String outputFile, double thr, double smt, boolean beatOD, boolean oaff)
   {
       this(inFiles, outputFile);
    ...
    public Segmenter(String inFile, String outputFile, double thr, boolean beatOD, boolean oaff)
    {
    ...
    public Segmenter(String inFile, String outputFile, double thr, double smt, boolean beatOD, boolean oaff)
    {
        this(inFile, outputFile);
    ...
    public FeatFile processAudioFile(String fn) throws IOException, UnsupportedAudioFileException
    {
        ...
        if(frameLength < 0)
        {
            frameLength = 50000*DpweOnsetDetector.swin/2;

        }
        int numFrames = (int) frameLength/(DpweOnsetDetector.swin/2);
        ...
        // use 30ms window
        stft = new STFT(ais, DpweOnsetDetector.swin, DpweOnsetDetector.swin/2, frameLatency);
        ...
        if(useBeatOD)
            ...
            detector = new DpweOnsetDetector(stft, numFrames, threshMult, smtime);
    ...
    public void run() 
    {
        ...
        for(int i=0; i<audioFiles.length; i++) 
        {
            ...
            if(verbose)
                System.out.println("Writing segments from " + audioFiles[i] + " to " 
                                   + outFileName + ".");
